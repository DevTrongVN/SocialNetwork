...
function switchTab(tabName) {
    ...
    if (!isOwnerGlobal && (tabName === 'requests' || tabName === 'saved' || tabName === 'hidden')) {
    ...
    ['posts', 'photos', 'videos', 'friends', 'following', 'requests', 'saved', 'hidden'].forEach(t => {
        const btn = document.getElementById(`tab-${t}`);
        if (btn) {
            // Logic ẩn Tab riêng tư nếu đang vào nhà người khác
            if (!isOwnerGlobal && ['requests', 'saved', 'hidden'].includes(t)) {
            ...
            if (t === tabName) {
                ...
                btn.classList.remove('text-themeSub', 'hover:bg-themeBg');
            } else {
                btn.classList.remove('bg-primary', 'text-white');
                btn.classList.add('text-themeSub', 'hover:bg-themeBg');
            }
        }
    });

    // Phát sáng nút "Thêm" nếu đang chọn Tab nằm trong Dropdown
    const moreBtn = document.getElementById("tab-more");
    if (moreBtn) {
        if (['following', 'requests', 'saved', 'hidden'].includes(tabName)) {
            moreBtn.classList.add('bg-primary', 'text-white');
            moreBtn.classList.remove('text-themeSub', 'hover:bg-themeBg');
        } else {
            moreBtn.classList.remove('bg-primary', 'text-white');
            moreBtn.classList.add('text-themeSub', 'hover:bg-themeBg');
        }
    }
    ...
    const createPostBox = document.getElementById("profileCreatePostBox");
    if (createPostBox) {
        if (tabName === 'posts' && isOwnerGlobal) createPostBox.classList.remove("hidden");
        else createPostBox.classList.add("hidden");
    }
    ...
    else if (tabName === 'hidden') loadHiddenPosts(token);
}
...
async function loadProfileInfo(userId, token, isOwner) {
    if (isOwner) {
        ...
        const profilePostAvt = document.getElementById("profilePostMyAvatar");
        if (profilePostAvt && res) profilePostAvt.outerHTML = getSafeAvatar(res.avatarUrl, res.username, "w-10 h-10 shadow-sm", "text-base");
        const profilePostInput = document.getElementById("profilePostContent");
        if (profilePostInput && res) profilePostInput.placeholder = `${res.username} ơi, bạn đang nghĩ gì thế?`;
        const hiddenTab = document.getElementById("tab-hidden"); if (hiddenTab) hiddenTab.classList.remove("hidden");
            ...
            if (window.location.hash) {
                setTimeout(() => {
                    ...
                    if (targetPost) {
                        ...
                        targetPost.classList.add('ring-4', 'ring-primary', 'transition-all', 'duration-500');
                        setTimeout(() => {
                            targetPost.classList.remove('ring-4', 'ring-primary');
                        }, 3000);
                    }
                }, 400);
            }
        ...
        const postCard = `
            <div id="post-${post.id}" class="bg-themePanel rounded-xl shadow-sm border border-themeBorder overflow-hidden transition-colors relative mb-4">
                <div class="absolute top-4 right-4 flex items-center gap-2 z-10">
                    ...
                    <button onclick="submitReport('Post', '${post.id}')" class="text-themeSub hover:text-red-500 transition px-1" title="Báo cáo bài viết"><i class="fa-solid fa-flag text-xl"></i></button>
                    ${untagBtn}
                    ${editDeleteMenu}
                </div>
            ...
            comments.forEach(c => {
                ...
                <div class="flex gap-2 ${marginClass} mb-2" id="comment-box-${c.id}">
                    <div class="cursor-pointer flex-shrink-0" onclick="location.href='profile.html?id=${c.userId}'">
                        ${getSafeAvatar(c.authorAvatar, c.authorName, c.parentCommentId ? "w-6 h-6" : "w-8 h-8", "text-[10px]")}
                    </div>
...
async function editComment(commentId, postId) {
    const currentContentElem = document.getElementById(`comment-content-${commentId}`);
    const currentText = currentContentElem.innerText;
    const newContent = prompt("Chỉnh sửa bình luận:", currentText);

    if (newContent !== null && newContent.trim() !== "" && newContent !== currentText) {
        const token = localStorage.getItem("jwtToken");
        try {
            const res = await fetch(`${BASE_URL}/api/post/comment/${commentId}`, {
                method: "PUT", headers: { "Authorization": "Bearer " + token, "Content-Type": "application/json" },
                body: JSON.stringify({ content: newContent.trim() })
            });
            if (res.ok) { loadComments(postId); } else { alert("Lỗi khi sửa!"); }
        } catch (e) { console.error(e); }
    }
}

function replyToComment(commentId, authorName, postId) {
    currentReplyToId = commentId;
    const input = document.getElementById(`comment-input-${postId}`);
    if (input) { input.value = `@[${authorName}](${authorName}) `; input.focus(); }
}
...
// --- HÀM XÓA VÀ SỬA BÀI VIẾT NÂNG CẤP ---
async function deletePost(postId) {
    ...
    try {
        ...
        if (res.ok) { alert("Đã xóa bài viết!"); switchTab('posts'); }
    } catch (e) { alert("Lỗi khi xóa bài."); }
}

async function editPost(postId) {
    const currentContentElem = document.getElementById(`post-content-${postId}`);
    const currentText = currentContentElem.innerText;

    const newContent = prompt("Chỉnh sửa nội dung bài viết:\n(Lưu ý: Không đổi được ảnh)", currentText);
    ...
    if (newContent !== null && newContent.trim() !== "" && newContent !== currentText) {
        ...
        try {
            const res = await fetch(`${BASE_URL}/api/post/${postId}`, {
                ...
                body: JSON.stringify({ content: newContent.trim(), privacy: 0 })
            });
            if (res.ok) {
                switchTab('posts');
            } else {
...
async function unhidePost(postId) {
    const token = localStorage.getItem("jwtToken");
    try {
        const res = await fetch(`${BASE_URL}/api/post/${postId}/hide`, { method: "DELETE", headers: { "Authorization": "Bearer " + token } });
        if (res.ok) switchTab('hidden');
    } catch (e) { console.error(e); }
}

function loadHiddenPosts(token) {
    fetch(`${BASE_URL}/api/post/hidden`, { headers: { "Authorization": "Bearer " + token } })
        .then(res => res.json())
        .then(posts => {
            const container = document.getElementById("tabContentContainer");
            if (posts.length === 0) { container.innerHTML = `<div class="bg-themePanel p-10 rounded-xl shadow-sm text-center border border-themeBorder text-themeSub font-medium"><i class="fa-solid fa-eye-slash text-4xl mb-3"></i><br>Bạn chưa ẩn bài viết nào.</div>`; return; }
            let html = '<div class="space-y-3">';
            posts.forEach(p => {
                html += `
                <div class="bg-themePanel p-4 rounded-xl border border-themeBorder flex justify-between items-center shadow-sm">
                    <div class="flex items-center gap-3">
                        ${getSafeAvatar(p.authorAvatar, p.authorName, "w-10 h-10")}
                        <div>
                            <div class="font-bold text-themeText">${p.authorName}</div>
                            <div class="text-xs text-themeSub line-clamp-1">${p.content || "Bài viết có chứa hình ảnh/video"}</div>
                        </div>
                    </div>
                    <button onclick="unhidePost('${p.id}')" class="bg-primary hover:opacity-80 text-white px-4 py-2 rounded-lg font-bold text-sm transition">Bỏ ẩn</button>
                </div>`;
            });
...
function previewProfileSelectedImage(event) {
    const file = event.target.files[0];
    if (file) {
        selectedProfileImageFile = file;
        const reader = new FileReader();
        reader.onload = function (e) {
            const container = document.getElementById("profileMediaPreviewContent");
            ...
            document.getElementById("profileImagePreviewContainer").classList.remove("hidden");
        }
...
function removeProfileImage() {
    ...
    document.getElementById("profileImagePreviewContainer").classList.add("hidden");
    const container = document.getElementById("profileMediaPreviewContent");
    if (container) container.innerHTML = "";
}

async function submitProfilePost() {
    const content = document.getElementById("profilePostContent").value;
    const btn = document.getElementById("btnSubmitProfilePost");
    const token = localStorage.getItem("jwtToken");
    const privacyVal = parseInt(document.getElementById("profilePostPrivacy")?.value || "0");

    if (!content && !selectedProfileImageFile) { alert("Bạn chưa nhập nội dung hoặc chọn file!"); return; }
    btn.innerText = "Đang xử lý..."; btn.disabled = true;

    try {
        let uploadedImageUrl = null;
        if (selectedProfileImageFile) {
            const previewContent = document.getElementById("profileMediaPreviewContent");
            if (!document.getElementById("profileUploadProgressBar")) {
                previewContent.insertAdjacentHTML('beforeend', `<div class="absolute bottom-0 left-0 h-1.5 bg-green-500 transition-all duration-300 z-50" id="profileUploadProgressBar" style="width: 0%"></div>`);
            }
            try {
                uploadedImageUrl = await uploadMediaWithProgress(selectedProfileImageFile, token, (percent) => {
                    btn.innerText = `Đang tải lên ${percent}%...`;
                    document.getElementById("profileUploadProgressBar").style.width = percent + "%";
                });
            } catch (uploadErr) {
